...
function render(data) {
  ...
  document.getElementById("suiteGrid").innerHTML = (data.suites || []).map(suite => `
    <article class="card">
  ...
  setHref("standaloneButton", data.standalonePanel.buttonUrl);

  setText("toolCatalogTitle", data.toolCatalog.title);
  document.getElementById("toolList").innerHTML = (data.toolCatalog.rows || []).map(row => `
    <div class="tool-row">
